...
function ServicesPage() {
  ...
  return (
    <PageFrame>
      <main>
        <PageIntro number="02" label="Services" title="One source of truth" copy="Separate teams create seams. We bring the designers, engineers and operational thinkers to one table so the idea survives every handoff." />
                 ...
                 <div className="flex flex-col gap-4 relative">
                   ...
                   {capabilities.map((cap, i) => (
                     ...
                     >
                       ...
                       <span className="font-display text-xl lg:text-3xl font-medium tracking-tight leading-none">{cap.title}</span>
                     </button>
                   ))}
                 </div>
               </div>
              ...
              {capabilities.map((cap, index) => (
                ...
                >
                  ...
                  >
                    ...
                    <h2 className="mt-4 font-display text-3xl md:text-4xl lg:text-5xl font-semibold tracking-tight">{cap.title}</h2>
                    <p className="mt-6 text-lg lg:text-xl text-ink/70 leading-relaxed max-w-[45ch]">{cap.copy}</p>

                    <div className="mt-12 pt-12 border-t border-line grid grid-cols-1 sm:grid-cols-2 gap-y-4 gap-x-8">
                       {cap.items.map((item) => (
                         <div key={item} className="flex flex-col gap-3 pb-3 border-b border-line group">
                           <span className="text-base font-medium text-ink/90 group-hover:text-reg transition-colors">{item}</span>
                         </div>
                       ))}
                    </div>
                  </motion.div>
          ...
          <div className="site-shell grid grid-cols-12 gap-6 py-24 lg:py-40 relative z-10">
            ...
            <Reveal className="col-span-12 lg:col-span-8 mt-8 lg:mt-0">
              <p className="max-w-[26ch] font-display text-3xl sm:text-4xl md:text-5xl leading-tight lg:text-7xl font-semibold tracking-tighter">
                The engineers understand the identity.<br />
                <span className="text-paper/40 italic font-serif">The designers understand the system.</span>
              </p>
            </Reveal>
          </div>
        </section>
